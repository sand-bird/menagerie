...
# the scale is found using IDEAL_SIZE, then clamped to the
# minimum and maximum possible scale just in case. if the
# window's width exceeds the maximum aspect ratio, we must
# calculate the scale using its height.
...
# our constraints. if the window is wider than the minimum
# aspect ratio, we must calculate base_size from the window's
# height; same with width if it's taller than the maximum.
# here we use width by default, and height when necessary.
