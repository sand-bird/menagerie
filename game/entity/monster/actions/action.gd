...
# run. 
# these are implemented by the base action with default values, and can be
# reimplemented by subclasses as needed.


# returns an estimation of the action's effect on the monster's drives, which is
# used by the Decider to calculate the action's utility.
func estimate_result(): return {}

# takes in the utility value generated by the Decider, based on the value of
# `estimate_result`, and returns a new utility value. can be used to modify the
# calculated utility value or simply override it. 
