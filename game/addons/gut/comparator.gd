...
func simple(v1, v2, missing_string=''):
	...
	if([TYPE_STRING, TYPE_STRING_NAME].has(tv1) and [TYPE_STRING, TYPE_STRING_NAME].has(tv2)):
		result.are_equal = v1 == v2
	elif(_utils.are_datatypes_same(v1, v2) or (
		_should_compare_int_to_float and
		[TYPE_INT, TYPE_FLOAT].has(tv1) and [TYPE_INT, TYPE_FLOAT].has(tv2)
	)):
		result.are_equal = is_equal_approx(v1, v2) if (tv1 == TYPE_FLOAT or tv2 == TYPE_FLOAT) else v1 == v2
