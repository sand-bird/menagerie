...
func _ready():
	connect("animation_finished", self, "play_next")
...
# each facing direction is a separate animation, so when our
# direction changes, we need to start playing the animation
# for the new one. in order to prevent it starting over from
# the beginning, we save our position from the old animation
# and seek to it in the new one.
...
func add(anim_id, anim_data):
	for y in anim_data.size():
		var anim = anim_data[y]
		if typeof(anim) == TYPE_ARRAY:
			for x in anim:
				add_facing(anim[x], str(anim_id, "_", y, "_", x))
		else:
			add_facing(anim, str(anim_id, "_", y, "_0"))
			add_facing(anim, str(anim_id, "_", y, "_1"), true)
...
func add_facing(anim_info, anim_name, flip = false):
